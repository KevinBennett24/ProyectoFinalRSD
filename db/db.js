...
function select(collection, id, empleadoid) {
    let data = collections[collection].data
    if(id) {
        return data.find(e => e.id === id && (empleadoid ? e.empleadoid === empleadoid : true))
    }else {
        return data.filter(e => empleadoid ? e.empleadoid === empleadoid : true)
    }
}
